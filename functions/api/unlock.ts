...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (!user) {
      return new Response(JSON.stringify({ success: false, message: "Verification failed. Incorrect details." }), { 
        status: 404, 
        headers: { ...corsHeaders, "Content-Type": "application/json" } 
      });
    }
    ...
    if (action === "SEND_OTP") {
      ...
      if (currentAttempts >= 3) {
        return new Response(JSON.stringify({ success: false, message: "Security limit reached. Try again later." }), { 
          status: 429, 
          headers: { ...corsHeaders, "Content-Type": "application/json" } 
        });
      }
      ...
      const emailHtml = getOTPTemplate(user.full_name, generatedOtp, "Account Unlock");
      
      try {
        await sendResendEmail(user.mail_id, "Action Required: Account Unlock Request", emailHtml);
      } catch (err: any) {
      ...
      return new Response(JSON.stringify({ success: true, message: "Unlock code sent to your email." }), { 
        status: 200, 
        headers: { ...corsHeaders, "Content-Type": "application/json" } 
      });
    }
    ...
    if (action === "VERIFY_UNLOCK") {
      ...
      return new Response(JSON.stringify({ success: true, message: "Account unlocked successfully." }), { 
        status: 200, 
        headers: { ...corsHeaders, "Content-Type": "application/json" } 
      });
    }
