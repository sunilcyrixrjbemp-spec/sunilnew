/**
 * ============================================================
 * CYRIX HEALTHCARE — DASHBOARD (HOME) BACKEND
 * File: home.js
...
async function resolveLoggedInUserId(request, url) {
    const candidates = [
...
export default async function homeHandler(request, env, corsHeaders) {
    ...
    try {
        /* ================================================================
           GET /api/home/list
           Dashboard Stats List: Admin sees their own AND team approvals
        ================================================================ */
        if (path === "/api/home/list" && method === "GET") {
            ...
            const user = await env.DB.prepare("SELECT role FROM user WHERE user_id = ?").bind(userId).first();
            ...
            if (isAdmin) {
                // Admin ko khud ka data OR jaha vo L1 approver hai wo My Expenses me dikhega
                query += ` AND (m.user_id = ? OR m.level_first_approver = ?)`;
                bindings.push(userId, userId);
            } else {
                // Normal users ko sirf khud ka data dikhega
                query += ` AND m.user_id = ?`;
        ...
        /* ================================================================
           ...
           Team Expenses List: Fetches subordinates data for Managers/Coordinators/Admins
        ================================================================ */
        if (path === "/api/approval/list" && method === "GET") {
            ...
            // Filter data purely based on the logical role hierarchy directly in SQL
            if (user.role === 'Manager') {
                // Manager ko sirf wahi dikhega jisme vo level_first_approver hai
                query += ` AND m.level_first_approver = ?`;
                bindings.push(userId);
            } else if (user.role === 'Coordinator') {
                // Coordinator ko sirf wahi dikhega jisme vo level_second_approver hai
                query += ` AND m.level_second_approver = ?`;
                bindings.push(userId);
            } else if (user.role === 'Admin' || user.role === 'Superadmin') {
                // Admin ko Team Expenses me jitna bhi expense submit hua hai vo SAB dikhega! 
                // Isliye yahan koi 'AND' filter nahi lagega.
            } else {
                // Baki kisi ke paas team nahi hai to empty return hoga
        ...
        /* ================================================================
           GET /api/home/detail
           Full Expense Details for the Summary Panel
        ================================================================ */
        if (path === "/api/home/detail" && method === "GET") {
            ...
            // Security Check: Ensure standard users can't fetch other peoples details unless they are the approver
            ...
            const enrichedItineraries = (itineraries || []).map(leg => {
                ...
                return {
                    ...
                    attachments, attachments_raw: undefined
                };
            });
