...
export default async function retriveHandler(request, env, corsHeaders) {
    ...
    try {
        ...
        if (!user) {
            return new Response(JSON.stringify({ success: false, message: "Record not found. Please check your details." }), { status: 404, headers });
        }
        ...
        if (action === "SEND_OTP") {
            const existing = await env.DB.prepare("SELECT attempts FROM otp_verifications WHERE user_id = ?").bind(user.user_id).first();
            let currentAttempts = existing ? existing.attempts : 0;
            
            if (currentAttempts >= 3) {
                return new Response(JSON.stringify({ success: false, message: "Security limit reached. Please try again later." }), { status: 429, headers });
            }
            ...
            await sendEmail(user.mail_id, "Security Alert: User ID Retrieval", emailTemplate, senderIdx);
        ...
        if (action === "VERIFY_RETRIEVE") {
            const stored = await env.DB.prepare("SELECT otp, expires_at FROM otp_verifications WHERE user_id = ?").bind(user.user_id).first();
            ...
            return new Response(JSON.stringify({ success: true, user_id: user.user_id }), { status: 200, headers });
        }
    } catch (e) { 
