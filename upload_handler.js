// ================================================================
// upload_handler.js — Cloudflare Workers Upload Report API
// Speed: D1 batch() — all rows in one round-trip, millisecond inserts
// Routes: /api/upload/asset | /api/upload/penalty | /api/upload/revenue
// ================================================================

export default async function uploadHandler(request, env, corsHeaders) {
    const url  = new URL(request.url);
    const path = url.pathname;
    if (request.method !== 'POST') return json({ error: 'Method not allowed' }, 405, corsHeaders);
    if (path === '/api/upload/asset')   return handleAssetUpload(request, env, corsHeaders);
    if (path === '/api/upload/penalty') return handlePenaltyUpload(request, env, corsHeaders);
    if (path === '/api/upload/revenue') return handleRevenueUpload(request, env, corsHeaders);
    return json({ error: 'Unknown upload route' }, 404, corsHeaders);
}

// ================================================================
// ASSET REPORT  —  unique: qr_code  |  update on conflict
// ================================================================
async function handleAssetUpload(request, env, corsHeaders) {
    try {
        ...
        if (miss) return json({ error: `Missing columns: ${miss}` }, 400, corsHeaders);

        // Fetch existing QR codes — insert vs update detect karne ke liye
        const existingRes = await env.DB.prepare('SELECT qr_code FROM asset_report').all();
        const existingQRs = new Set((existingRes.results || []).map(r => r.qr_code));

        const successLog = [], failLog = [];
        const stmts = [];
        ...
        // D1 batch — one round-trip per chunk, max speed
        const D1_LIMIT = 1000;
        ...
        for (let i = 0; i < stmts.length; i += D1_LIMIT) {
            ...
            results.forEach((res, j) => {
                ...
                if (res.meta && res.meta.changes > 0) {
                    ...
                    // changes=0 means data same tha — no-op
                    successLog.push({ row: s.rowNum, id: s.qr, status: 'No change' });
                }
            });
        }

        return json({ success: true, inserted, updated, skipped: failLog.length, errors: 0, successLog, failLog }, 200, corsHeaders);
    } catch (e) {
...
// ================================================================
// PENALTY REPORT  —  unique: complaint_id  |  update on conflict
// ================================================================
async function handlePenaltyUpload(request, env, corsHeaders) {
    try {
        const { rows: dataRows, header, error } = await parseRequest(request);
        if (error) return json({ error }, 400, corsHeaders);
        ...
        // Fetch existing IDs in one query to detect insert vs update
        const existingRes = await env.DB.prepare('SELECT complaint_id FROM penalty_report').all();
        const existingIds = new Set((existingRes.results || []).map(r => r.complaint_id));
...
// ================================================================
// REVENUE REPORT
// unique: (di_name, eligibility_month)  ← COMPOSITE KEY
// Ek DI ka ek month = ek row
// Same month mein multiple districts/DIs allowed
// Same DI + same month → UPDATE (penalty jaisa)
// ================================================================
async function handleRevenueUpload(request, env, corsHeaders) {
    try {
        const { rows: dataRows, header, error } = await parseRequest(request);
        if (error) return json({ error }, 400, corsHeaders);
        ...
        // Fetch existing (di_name, eligibility_month) pairs — insert vs update detect
        const existingRes = await env.DB.prepare(
            'SELECT di_name, eligibility_month FROM revenue_report'
        ).all();
        const existingKeys = new Set(
            (existingRes.results || []).map(r => `${r.di_name}||${r.eligibility_month}`)
        );

        const successLog = [], failLog = [];
        const stmts = [];

        // In-CSV duplicate guard
