// ============================================================
//  DASHBOARD HANDLER — Ultra Fast & Secured Aggregation Version
...
// CACHE CONFIGURATION (Makes API responses instant for repeat visits)
const CACHE_HEADERS = {
    'Cache-Control': 'private, max-age=30, stale-while-revalidate=60', // Cache for 30 seconds
    'X-Content-Type-Options': 'nosniff'
};

// Facility table name caching to avoid checking DB every request
let cachedFacTable = null;
...
export default async function dashboardHandler(request, env, corsHeaders) {
    ...
    try {
        if (!env.DB) return new Response(safeStringify({ success: false, message: 'Database binding missing.' }), { status: 500, headers });

        /* ── 1. RESOLVE & VALIDATE USER (Targeted Fetch - Extremely Fast) ── */
        const userId = await resolveUserId(request, url);
        if (!userId) return new Response(safeStringify({ success: false, message: 'Unauthorized.' }), { status: 401, headers });

        // ONLY fetch the requesting user (Removed SELECT * FROM user bloat)
        const user = await env.DB.prepare(`SELECT user_id, full_name, role, e_upkaran_id, zone_name, district_name FROM user WHERE user_id = ?`).bind(userId).first();
        if (!user) return new Response(safeStringify({ success: false, message: 'User not found.' }), { status: 403, headers });
        ...
        /* ── Determine Facility Table ONCE ── */
        if (!cachedFacTable) {
            try { await env.DB.prepare(`SELECT 1 FROM "Facily Details" LIMIT 1`).first(); cachedFacTable = '"Facily Details"'; }
            catch { cachedFacTable = 'facility_details'; }
        }

        /* ── 2. FILTERS API ────────────────────────────────────── */
        if (path.endsWith('/filters') && method === 'GET') {
            // Fetch dropdown lists concurrently (Parallel Execution)
        ...
        /* ── DYNAMIC SQL BUILDERS (OPTIMIZED WITH JOINS) ── */
        function getPenaltySQL() {
            let conds = ['1=1'];
            let params = [];
            
            // Replaced Subqueries with optimized fields using JOIN
            if (!isAdmin) {
        ...
        /* ════════════════════════════════════════════════════════
           3. PENALTIES API (Parallel Execution)
        ════════════════════════════════════════════════════════ */
        if (path.endsWith('/penalties') && method === 'GET') {
            const pSQL = getPenaltySQL();
            try {
                ...
                if (pSQL.params.length > 0) {
                    rawStmt = rawStmt.bind(...pSQL.params);
                    aggStmt = aggStmt.bind(...pSQL.params);
                }

                // RUN CONCURRENTLY FOR 2x SPEED
            ...
            } catch (e) { console.error('Penalty error:', e.message); return new Response(safeStringify({ success: false, message: e.message }), { status: 500, headers }); }
        }

        /* ════════════════════════════════════════════════════════
           4. EXPENSES API (Parallel Execution)
        ════════════════════════════════════════════════════════ */
        if (path.endsWith('/expenses') && method === 'GET') {
            ...
            } catch (e) { console.error('Expense error:', e.message); return new Response(safeStringify({ success: false, message: e.message }), { status: 500, headers }); }
        }
